#' #' Build EndNote xml from MS Forms data
#'
#' Build xml in \code{\link{xmlGenerator}}
#'
#' @param record_list Required. A list object produced by \code{"load_data()"} or \code{"load_example_data()"}.
#' @param write Optional boolean. Default = \code{FALSE}. TRUE produces a dialog box for the user to save their xml output. **NOTE** The user must provide the file extension .xml
#'
#' @return \code{xml_output}: xml document of data parsed from \code{"record_list"}.
#' @return \code{xml}: Optional, only available if \code{write = TRUE}. A .xml file, saved to the user's computer, that can be imported into EndNote to add records to an EndNote library.
#'
#' @examples
#' xmlGenerator::build_xml(record_list) # `write` defaults to FALSE
#' xmlGenerator::build_xml(record_list, write = TRUE)
#' # provide a filename in dialog box. E.g., my_xml.xml
#'
#' @export

build_xml <- function(record_list, write = FALSE){
    tryCatch(
        expr = {
